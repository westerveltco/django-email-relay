...
@nox.session(python=PY_LATEST)
def test_lts(session):
    for django in DJ_LTS:
        session.notify(f"tests(python='{PY_DEFAULT}', django='{django}')")


@nox.session(python=PY_LATEST)
def test_latest(session):
    session.notify(f"tests(python='{PY_LATEST}', django='{DJ_LATEST}')")
